...
from api.extensions import bcrypt, limiter, login_manager
...
def login():
  try: 
    ...
    return jsonify({"success": False, "message": "Invalid credientials!"}), 400
...
def signup():
  ...
  try: 
    ...
    name = signup_attempt.user.strip().lower()
    password = signup_attempt.password
        
    with SessionLocal() as session:
      user_exists = session.scalar(select(User).where(User.name==name))
      if user_exists:
        return jsonify({"success": False, "message": "Unable to create account!"}), 409
